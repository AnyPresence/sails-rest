...
DefaultFormatterDelegate.prototype.getResultsAsCollection = function(data, collectionName, config, id, callback) {
  var d = (data.objects || data.results || data),
      a = _.isArray(d) ? d : [d];

  return this.formatResults(a, collectionName, config, id, callback);
};
